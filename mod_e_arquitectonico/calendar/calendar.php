...
//espacio en blanco
      ...
      $numero=0;
      $consulta = "select * from eventcalenderelementoarquitectonico where eventDate = '".$todaysDate."' and idequipo='".$ids."' and checkbox= '".$numero."'";
      ...
      //identifica dias con evento
        $sqlCount = "select * from eventcalenderelementoarquitectonico where eventDate = '".$dateToCompare."' and idequipo='".$ids."'";
...
echo'><a href="../calendar/reenvio.php?id='.($row["idequipo"]).'&month='.$monthstring.'&day='.$daystring.'&year='.$year.'" '; ?> target="popup" onClick="window.open(this.href, this.target, 'width=400,height=350'); return false;"  <?php echo'> '.$i.'</a></td>';
...
<form method="POST" action="../fichaestructura.php">
        ...
        <button type="button" class="button2" style="BORDER: rgb(128,128,128) 1px none; FONT-SIZE: 10pt; FONT-FAMILY: Verdana;" value="Enviar" title="Ver" onclick="form.submit()">Ficha Equipo</button>
    </form>
   </div>
